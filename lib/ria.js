...
Ria.prototype.getModuleItem = function(moduleName, id, callback, returnFormat) {
	returnFormat = returnFormat || 'xml';
	return this._get('/ria-ws/application/module/' + moduleName + '/' + id, callback, returnFormat);
}

Ria.prototype.getModuleItemPromise = function(moduleName, id, returnFormat) {
	var self = this;
	return new Promise(function(resolve, reject) {
		self.getModuleItem(moduleName, id, (err, res, body) => {
			if (err) {
				reject({err, res, body});
			} else {
				resolve({err, res, body});
 ...
 * @param  {Object}   args         args.limit and args.offset, args.sort
